...
const ALL_PREVIOUS_YEAR_QUESTIONS = [
  {
    id: "1",
    title: "DSA - 2023 PYQ",
    description: "Previous year questions for Data Structures and Algorithms (2023).",
    type: "PYQ",
    downloadLink: "/placeholder.pdf?query=dsa-2023-pyq",
    previewLink: "/placeholder.pdf?query=dsa-2023-pyq-preview",
  },
  {
    id: "2",
    title: "OS - 2022 PYQ",
    description: "Previous year questions for Operating Systems (2022).",
    type: "PYQ",
    downloadLink: "/placeholder.pdf?query=os-2022-pyq",
    previewLink: "/placeholder.pdf?query=os-2022-pyq-preview",
  },
  {
    id: "3",
    title: "DBMS - 2021 PYQ",
    description: "Previous year questions for Database Management Systems (2021).",
    type: "PYQ",
    downloadLink: "/placeholder.pdf?query=dbms-2021-pyq",
    previewLink: "/placeholder.pdf?query=dbms-2021-pyq-preview",
  },
  {
    id: "4",
    title: "CN - 2020 PYQ",
    description: "Previous year questions for Computer Networks (2020).",
    type: "PYQ",
    downloadLink: "/placeholder.pdf?query=cn-2020-pyq",
    previewLink: "/placeholder.pdf?query=cn-2020-pyq-preview",
  },
];

export default function PreviousYearQuestionsPage() {
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredPYQs, setFilteredPYQs] = useState(ALL_PREVIOUS_YEAR_QUESTIONS);

  useEffect(() => {
    if (searchTerm === "") {
      setFilteredPYQs(ALL_PREVIOUS_YEAR_QUESTIONS);
    } else {
      const lowerCaseSearchTerm = searchTerm.toLowerCase();
      const filtered = ALL_PREVIOUS_YEAR_QUESTIONS.filter(item =>
    ...
    <div className="flex min-h-screen flex-col items-center py-12 px-4 sm:px-6 lg:px-8 bg-background text-foreground"> {/* Changed bg-white dark:bg-gray-900 to bg-background text-foreground */}
      <div className="w-full max-w-5xl space-y-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold mb-2">Previous Year Questions</h1> {/* Removed explicit text-gray-900 dark:text-gray-50 */}
          <p className="text-lg text-gray-600 dark:text-gray-400">
            Access past exam papers and solutions to prepare for your exams.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPYQs.length > 0 ? (
            ...
            <p className="col-span-full text-center text-gray-500 dark:text-gray-400">No previous year questions found matching your search.</p>
          )}
        </div>
      </div>
    </div>
